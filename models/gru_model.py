...
class GRUModel(pl.LightningModule):
    """
    사전 학습된 모델과 Bi-directional GRU layer를 결합하여
    시퀀스 분류 작업을 수행하는 PyTorch Lightning 모델.
    ...
    Args:
        ...
        bce (bool, optional): 평가에 이진 교차 엔트로피(Binary Cross Entropy, BCE)를
            사용할지 여부. 기본값은 False
    ...
    Attributes:
        ...
        evaluation (PearsonCorrCoef 또는 F1Score): 평가 지표로,
            피어슨 상관 계수(Pearson Correlation Coefficient) 또는 F1 점수(F1 Score)를 사용
    ...
    def __init__(self,
        ...
        self.gru = GRU(input_size=2 * self.plm.config.hidden_size,
                       ...
                       batch_first=True,
                       bidirectional=True)
        self.linear = Linear(in_features=2 * self.plm.config.hidden_size,
                             out_features=1)
        self.tanh = Tanh()
        ...
        if self.loss_function == 'SmoothL1Loss':
            self.loss_func = getattr(torch.nn,
                                     self.loss_function)(beta=self.beta)
        else:
    ...
    def forward(self, x: Tensor) -> Tensor:
        """GRU model의 forward pass.

        Input Tensor가 pre-trained 모델을 통과해 bidirectional GRU layer와
        linear layer를 통과.
        ...
        last_gru_state = last_gru_state.view(3, 2, -1,
                                             self.plm.config.hidden_size)
        # Concatenate the last hidden state of forward and backward GRU
        last_hidden_state = torch.cat((last_gru_state[-1, 0],
                                       last_gru_state[-1, 1]), dim=-1)
    ...
    def training_step(self,
        ...
        Args:
            batch (Tuple[Tensor, Tensor]): 토큰화된 input sequence와 label을
                포함하는 텐서 튜플.
